...
def apply_filters(query, filters, company_currency, salary_slip_doc):
...
def get_data(filters,company_currency):
	...
	query = frappe.qb.from_(salary_slip_doc)\
		...
		.select(
			...
			employee_doc.custom_national_id, 
	...
	query= apply_filters(query, filters, company_currency, salary_slip_doc)
